...
test('Round 3 native IDB: read-only diagnostic preserves seeded data and never calls Store',{timeout:15000},async()=>{
 const h=await uiHarness();try{const p=await h.page();const result=await p.evaluate(async code=>{
  ...
  let report;const logs=[];try{report=await new Function('globalThis','IDBKeyRange','console','return (\n'+code+'\n);')({indexedDB,chrome:{runtime:{id:'synthetic'}},location:{protocol:'chrome-extension:',host:'synthetic'}},IDBKeyRange,{info:x=>logs.push(x)});}finally{IDBDatabase.prototype.transaction=original;}
...
test('Round 3 native IDB: create-after-enumeration race is aborted with no database left behind',{timeout:15000},async()=>{
 const h=await uiHarness();try{const p=await h.page();const r=await p.evaluate(async code=>{
  ...
  const report=await new Function('globalThis','IDBKeyRange','console','return (\n'+code+'\n);')({indexedDB:factory,chrome:{runtime:{id:'synthetic'}},location:{protocol:'chrome-extension:',host:'synthetic'}},IDBKeyRange,{info:x=>logs.push(x)});
