...
test('ANS-08 Chrome windowing preserves dirty, IME, selection and bounded save/undo state',{timeout:180000},async()=>{
 ...
 try{
  ...
  await page.evaluate(()=>{const send=chrome.runtime.sendMessage.bind(chrome.runtime);window.ans08Edit={failEdits:false,tracked:[],failedBatchSizes:[],savedBatchSizes:[],historySizes:[]};window.ans08EditRestore=()=>chrome.runtime.sendMessage=send;chrome.runtime.sendMessage=(message,...args)=>{if(message?.type==='GET_LIBRARY_TRACKED_ENTRIES')window.ans08Edit.tracked.push(message.options?.ids?.length||0);if(message?.type==='EDIT_LIBRARY_BATCH'){const n=message.edit?.entries?.length||0;if(window.ans08Edit.failEdits){window.ans08Edit.failedBatchSizes.push(n);return Promise.resolve({ok:false,error:'STORAGE_FAILED'});}window.ans08Edit.savedBatchSizes.push(n);}if(message?.type==='THOUGHT_EDIT_HISTORY')window.ans08Edit.historySizes.push(message.edit?.items?.length||0);return send(message,...args);};});
  ...
  await eventually(async()=>{const sizes=await page.evaluate(()=>window.ans08Edit.savedBatchSizes);return sizes.length>=3&&sizes.every(n=>n>0&&n<=40);},'bounded retry save batches',30000);
  const savedBody=await rpc(page,'GET_LIBRARY_ENTRY',{id:setup.dirtyId});assert.match(savedBody.body,/DIRTY_000 RECOVER/);
  ...
  const undoTarget=setup.dirtyIds.at(-1),beforeUndo=(await rpc(page,'GET_LIBRARY_ENTRY',{id:undoTarget})).body;await page.locator('[data-entry-id="'+undoTarget+'"] [data-entry-field="body"]').focus();await page.keyboard.press(process.platform==='darwin'?'Meta+z':'Control+z');
  await eventually(async()=>(await rpc(page,'GET_LIBRARY_ENTRY',{id:undoTarget})).body!==beforeUndo,'bounded undo',20000);
  ...
  const evidence={trackedBatches:await page.evaluate(()=>[...window.ans08Edit.tracked]),failedBatchSizes:await page.evaluate(()=>[...window.ans08Edit.failedBatchSizes]),savedBatchSizes:saveSizes,historySizes,dirtyNodePreserved:afterIme.dirtySame,imeNodePreserved:afterIme.imeSame,imeCaret:afterIme.caret,selectionPreserved:selectionAfter.same,sourceDigestBefore:sourceBefore,sourceDigestAfter:sourceAfter,reverseEdit:false,externalRequests:h.externalRequests,extensionNetworkRequests:h.extensionNetworkRequests,deepSeekRequests:h.deepSeekRequests.length};
