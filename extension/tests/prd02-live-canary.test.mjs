...
const diagnostics={adapterVersion:'0.3.0',status:'CAPTURING',recentCapture:true,scanned:6,structure,ingestion,captureHealthState:'ACCEPTED',lastErrorCode:null,structuralRejections:[]};
...
test('live structure must accept every visible user role and match ingestion exactly',()=>{
 ...
 assert.equal(summarizePassive({runtime,observation,diagnostics:{...diagnostics,scanned:5},archive}).pass,false);
});
...
test('structural rejection diagnostics expose only bounded body-free flags',()=>{
 const rejected={idOnRole:false,idOnAncestor:true,idOnDescendant:false,editorCheckAvailable:false,rootInsideEditor:false,visibleEditorsInTurn:0,busy:false,textMatches:0,ownedTextMatches:0,visibleTextMatches:0,safeTextMatches:0};
 const r=summarizePassive({runtime,observation,diagnostics:{...diagnostics,status:'ADAPTER_MISMATCH',structure:{...structure,finalCandidateCount:5},ingestion:{...ingestion,attempted:5,duplicates:5,knownTimes:4,unknownTimes:1},scanned:5,structuralRejections:[rejected]},archive});
 assert.deepEqual(r.diagnostics.structuralRejections,[rejected]);
 assert.equal(JSON.stringify(r).includes('private-chat'),false);
});
...
test('passive verifier reads only body-free indexes/meta and launcher never mutates or reloads PAIA',async()=>{
 ...
 assert.match(command,/json\.dumps\(data,separators=/);
 assert.doesNotMatch(command,/PAIA_PRD02_RUNTIME_CHECK=%s;\\\\n/);
