...
async function assertShell(p){
 ...
 assert.equal(await p.locator('#universal-search-open').isVisible(),false,'normal Archive shell has no visible global search launcher');assert.equal(await p.locator('#universal-search-open').getAttribute('aria-hidden'),'true');assert.equal(await p.locator('#search').isVisible(),true,'Archive root owns the visible search box');
