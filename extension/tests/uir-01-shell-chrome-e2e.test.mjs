...
async function assertShell(p){
 ...
 assert.equal(await p.locator('#uir-revisit-row').count(),1,'Archive composes the two historic Revisit contracts into one visual row');assert.equal(await p.locator('#revisit-open').isVisible(),true,'the direct Revisit contract remains visible and clickable');assert.equal(await p.locator('#core-loop-return').isVisible(),false,'legacy return card is absent from the new Archive root');assert.equal((await p.locator('#core-loop-return .core-loop-card-label').textContent()).trim(),'本机变化');assert.equal(await p.locator('#core-loop-return > strong').isVisible(),false,'the status card does not repeat a second visible Revisit label');
 assert.equal(await p.locator('#uir-archive-main').count(),1);assert.equal(await p.locator('#uir-archive-assist').isVisible(),false,'Archive root has no default side dashboard');assert.equal(await p.evaluate(()=>location.hash+location.search),'','UI refresh must not invent URL routes');
}
...
async function screenshotArchiveMatrix(p){
 ...
 for(const [width,height,theme] of [[1440,900,'light'],[1440,900,'dark'],[1024,768,'light'],[390,844,'light']]){
  ...
  const main=await p.locator('#uir-archive-main').boundingBox();assert.ok(main);assert.equal(await p.locator('#uir-archive-assist').isVisible(),false);
