...
test('Passport-protected release is fail-closed and once grant cannot release text twice',async()=>{
 ...
 const built=buildResult();await signals.observe(buildRequest,built,{url:'chrome-extension://test/ui/product-signals.html'});assert.equal(built.contextPackage.persistedBody,false);
 const grant=await signals.settings({passport:{action:'create',consumer:'chatgpt',purpose:'research',profileId:'default',duration:'once'}});
 const first={text:'authorized synthetic context',characters:28,tokens:12,generation:5,localOnly:true};await signals.observe({type:'PAIA_MEMORY_SHARE',options:{previewId:built.previewId,format:'copy',removed:[],grantId:grant.grantId}},first,{});assert.equal(first.permissionDenied,false);assert.equal(first.text,'authorized synthetic context');assert.equal(first.contextPackage.consumer,'chatgpt');assert.equal(first.contextPackage.purpose,'research');
...
test('revoked grant cannot release reconstructed plaintext and Passport metadata is excluded from PAIA Backup',async()=>{
 ...
 const grant=await signals.settings({passport:{action:'create',consumer:'claude',purpose:'writing',profileId:'default',duration:'30d'}});await signals.settings({passport:{action:'revoke',grantId:grant.grantId}});
...
test('manual explicit AI Context share remains compatible and receives a package contract',async()=>{
 const {s}=await setup(OrganizerStore,{clock:()=>new Date(now).toISOString()});const memory=new MemoryService(s,{clock:()=>now});await memory.ready();let n=0;const signals=new ProductSignals(s,{clock:()=>now,uuid:()=>`manual-${++n}`});const built=buildResult();await signals.observe(buildRequest,built,{});const result={text:'manual context',characters:14,tokens:6,generation:5,localOnly:true};await signals.observe({type:'PAIA_MEMORY_SHARE',options:{previewId:built.previewId,format:'copy',removed:[]}},result,{});assert.equal(result.text,'manual context');assert.equal(result.contextPackage.consumer,'manual');assert.equal(result.contextPackage.purpose,'current_task');const status=await signals.status();assert.ok(status.passport.audits.some(a=>a.action==='manual_copy'));
});
