import test from 'node:test';import assert from 'node:assert/strict';import {mkdir,writeFile} from 'node:fs/promises';import {execFileSync} from 'node:child_process';
...
test('fixed coverage corpus: pure whole-input productions improve synthetic coverage with zero high-risk false filters',async()=>{
 const source=execFileSync('git',['show','1e00c23:core/smart-filter.js'],{encoding:'utf8'}),old=await import('data:text/javascript;base64,'+Buffer.from(source).toString('base64'));const cases=coverageCases();const reports={baseline:{filtered:0,falseFiltered:0},current:{filtered:0,falseFiltered:0}};const reasonCounts={};let missed=0;
 for(const c of cases){const result=decideLight(c);reasonCounts[result.reasonCode]=(reasonCounts[result.reasonCode]||0)+1;for(const [name,classifier]of [['baseline',old.decideLight],['current',decideLight]]){const yes=classifier(c).decision==='filter';reports[name].filtered+=Number(yes);reports[name].falseFiltered+=Number(yes&&c.label!=='pure_control');}missed+=Number(c.label==='pure_control'&&result.decision!=='filter');assert.equal(result.decision==='filter',c.label==='pure_control','synthetic case '+c.id);}
