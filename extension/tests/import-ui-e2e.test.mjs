...
test('isolated test-only adapter exercises real background IDB batches, paused file reselection and dedupe',{timeout:60000},async()=>{
 ...
 try{
  ...
  await h.archive.locator('#sync-history').click();await pick();assert.equal((await h.state()).records.length,0);
  await h.archive.locator('#history-pause').click();
  await eventually(async()=>(await h.archive.locator('#history-status').textContent())==='已暂停。重新选择同一文件即可继续。');
  await pick();
  const resumed=await h.archive.evaluate(()=>chrome.runtime.sendMessage({type:'IMPORT_TASKS'}));
  assert.equal(resumed.data.tasks.length,1);
  assert.equal((await h.state()).records.length,0);
  await h.archive.locator('#history-close').click();await h.archive.locator('#history-dialog').waitFor({state:'hidden'});
