...
test('UX-R5 ON-01 keeps Original readable, scopes generation to one Topic and reuses cache without provider calls',{timeout:150000},async()=>{
 ...
 try{
  ...
  await p.emulateMedia({reducedMotion:'reduce'});await p.bringToFront();await p.evaluate(()=>{const toggle=document.getElementById('ai-presentation-toggle');window.__r5FocusRestores=[];new MutationObserver(mutations=>{if(mutations.some(m=>m.attributeName==='disabled')&&!toggle.disabled)window.__r5FocusRestores.push(document.activeElement===toggle);}).observe(toggle,{attributes:true,attributeFilter:['disabled']});});assert.equal(await toggle.evaluate(el=>{el.focus();return document.activeElement===el;}),true,'AI switch accepts focus before transition');await toggle.uncheck();await original.waitFor();await eventually(async()=>await p.evaluate(()=>window.__r5FocusRestores?.at(-1)===true),'Original switch restores toggle focus');assert.equal(await p.evaluate(()=>document.documentElement.classList.contains('paia-recomposing')),false);assert.equal(h.deepSeekRequests.length,1);const restoredCount=await p.evaluate(()=>window.__r5FocusRestores.length);await toggle.check();await p.locator('[data-ai-field="blockSummary"]').filter({hasText:'R5 AI 摘要 1'}).waitFor();await eventually(async()=>await p.evaluate(count=>window.__r5FocusRestores?.length>count&&window.__r5FocusRestores.at(-1)===true,restoredCount),'cached Organized switch restores toggle focus');assert.equal(h.deepSeekRequests.length,1,'cached switch is local only');
