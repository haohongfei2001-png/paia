...
async function authorizeFirstGeneration(p){await p.getByRole('button',{name:'生成 AI整理',exact:true}).click();const dialog=p.locator('#library-dialog');await dialog.waitFor({state:'visible'});await dialog.getByRole('button',{name:'授权并开始',exact:true}).click();}

test('UX-R5 Original -> explicit first AI generation -> cached topic-local view',{timeout:150000},async()=>{
 ...
 try{
  ...
  const toggle=p.locator('#ai-presentation-toggle');await eventually(()=>toggle.isEnabled(),'Topic AI switch enabled with zero provider calls');await toggle.check();await p.locator('[data-ai-first-generation]').waitFor();assert.equal(h.deepSeekRequests.length,0,'switching to Organized must not call DeepSeek');assert.equal(await original.isVisible(),true,'Original stays visible before first generation');
  await p.screenshot({path:`${dir}/first-generation.png`,fullPage:true});
  await authorizeFirstGeneration(p);await eventually(()=>Promise.resolve(h.deepSeekRequests.length===1),'first paid request');assert.equal(requestOf(h.deepSeekRequests[0]).inputs.length,1);assert.equal(await original.isVisible(),true,'Original must stay readable while first generation is pending');
  ...
  await p.emulateMedia({reducedMotion:'reduce'});await toggle.focus();await toggle.uncheck();await original.waitFor();assert.equal(await p.evaluate(()=>document.activeElement?.id),'ai-presentation-toggle');assert.equal(await p.evaluate(()=>document.documentElement.classList.contains('paia-recomposing')),false);assert.equal(h.deepSeekRequests.length,1);
  await toggle.check();await p.locator('[data-ai-field="blockSummary"]').filter({hasText:'R5 AI 摘要 1'}).waitFor();assert.equal(await p.evaluate(()=>document.activeElement?.id),'ai-presentation-toggle');assert.equal(h.deepSeekRequests.length,1,'cached view switch must not call DeepSeek');
  await p.locator('#back').click();await p.locator(`[data-topic-id="${topic.id}"]`).click();await p.locator('[data-ai-field="blockSummary"]').filter({hasText:'R5 AI 摘要 1'}).waitFor();assert.equal(await toggle.isChecked(),true,'same-tab Topic view should stay Organized');assert.equal(h.deepSeekRequests.length,1);assert.equal(h.externalRequests,0);assert.deepEqual(h.errors,[]);
 }finally{await h.close();}
});

test('UX-R5 leaving a Topic during one paid request never starts a second request',{timeout:90000},async()=>{
 ...
 try{
  const p=await ready(h),topic=await createTopic(p,'R5 进行中离开');await openTopic(p,topic);await p.locator('#ai-presentation-toggle').check();await authorizeFirstGeneration(p);await eventually(()=>Promise.resolve(h.deepSeekRequests.length===1),'in-flight request');
  await p.locator('#back').click();assert.equal(h.deepSeekRequests.length,1);release();await eventually(async()=>{const {topic:row,state}=await aiStatus(p,topic.id);return !!row?.presentation||['failed','committed','outcome_unknown'].includes(state.runtime?.state);},'terminal AI state',30000);await pause(300);assert.equal(h.deepSeekRequests.length,1);assert.equal(h.externalRequests,0);assert.deepEqual(h.errors,[]);
 }finally{await h.close();}
});

test('UX-R5 worker interruption becomes outcome_unknown and status checks never retry',{timeout:90000},async()=>{
 let release;const gate=new Promise(resolve=>{release=resolve;});let call=0;
 ...
 try{
  const p=await ready(h),topic=await createTopic(p,'R5 outcome unknown');await openTopic(p,topic);await p.locator('#ai-presentation-toggle').check();const requestPromise=authorizeFirstGeneration(p).catch(()=>{});await eventually(()=>Promise.resolve(h.deepSeekRequests.length===1),'request crossed network boundary');
  await h.restartWorker();release();await requestPromise;await eventually(async()=>{const state=await rpc(p,'GET_AI_PRESENTATION_STATUS');return state.runtime?.state==='outcome_unknown';},'outcome unknown after restart');const before=h.deepSeekRequests.length;await rpc(p,'GET_AI_PRESENTATION_STATUS');await rpc(p,'GET_AI_PRESENTATION_STATUS');await pause(300);assert.equal(h.deepSeekRequests.length,before);assert.equal(h.externalRequests,0);assert.deepEqual(h.errors,[]);
 }finally{await h.close();}
});
...
test('UX-R5 Source purge removes source-bound AI output without regeneration',{timeout:90000},async()=>{
 ...
 try{
  ...
  await openTopic(p,sourceTopic);await p.locator('#ai-presentation-toggle').check();await authorizeFirstGeneration(p);await eventually(async()=>{const {topic:row}=await aiStatus(p,sourceTopic.id);return !!row?.presentation;},'source-bound AI presentation');assert.equal(h.deepSeekRequests.length,1);const source=(await rpc(p,'GET_INPUT',{id:inputId})).originalTextReference;await rpc(p,'PURGE_SOURCE',{id:source,confirm:true});await eventually(async()=>{const {topic:row}=await aiStatus(p,sourceTopic.id);return !row?.presentation;},'purge clears derived AI presentation');await pause(300);assert.equal(h.deepSeekRequests.length,1);assert.equal(h.externalRequests,0);assert.deepEqual(h.errors,[]);await chat.close();
 }finally{await h.close();}
});
