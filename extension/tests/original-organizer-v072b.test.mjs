...
test('v072b exact original remains one shared protected body across later Input revisions',async()=>{
 const f=await fixture();await f.runner.wake({manual:true});await f.s.reverseEditSetting(true);const first=(await f.s.entryPage()).items[0];await f.s.editEntry({id:first.id,expectedRevision:first.revision,expectedInputRevision:first.currentInputRevision,operationId:crypto.randomUUID(),changes:{body:'User kept wording'}});assert.equal((await f.s.input(f.input.id)).libraryText,'User kept wording');await inputEdit(f.s,f.input.id,{libraryText:'Later source wording'});await f.runner.wake({manual:true});const entries=await f.s.entryPage();assert.equal(entries.items.filter(x=>x.id===first.id).length,1);assert.equal((await f.s.entry(first.id)).body,'Later source wording');const protectedEntry=await f.s.entry(first.id);assert.equal(protectedEntry.protections.body.locked,true);
});
