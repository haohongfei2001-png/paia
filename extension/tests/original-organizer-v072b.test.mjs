...
test('v072b direct user edits remain protected when a later Input revision is organized',async()=>{
 const f=await fixture();await f.runner.wake({manual:true});const first=(await f.s.entryPage()).items[0];await f.s.editEntry({id:first.id,expectedRevision:first.revision,operationId:crypto.randomUUID(),changes:{body:'User kept wording'}});await inputEdit(f.s,f.input.id,{libraryText:'Later source wording'});await f.runner.wake({manual:true});const entries=await f.s.entryPage();assert.ok(entries.items.some(x=>x.body==='User kept wording'));assert.ok(entries.items.some(x=>x.body==='Later source wording'));const protectedEntry=await f.s.entry(first.id);assert.equal(protectedEntry.protections.body.locked,true);
});
