...
test('Round 8 browser: opt-in direct Input Context, no denied-topic bypass, and one working body edits from Input or Thought',{timeout:120000},async()=>{
  ...
  try{
    ...
    // Input -> same Thought. Other browser journeys already cover pointer
    // stability on the collection list; this feature journey dispatches the real
    // row click synchronously so a concurrent maintenance refresh cannot cause a
    // Playwright-only actionability retry loop.
    ...
    await p.locator(`.conversation-document[data-document-id="${docId}"]`).evaluate(el=>el.click());
    await eventually(async()=>await p.locator('#document-panel').isVisible()&&await p.locator('.library-prose').count()===1,'Input document opens');
