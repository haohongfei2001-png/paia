...
test('Smart Filter isolated Chrome: capture, Light reading, full search/context, keep, Off and no document controls',{timeout:90000},async()=>{
 const h=await FakeChatGPT.start({headless:false});try{
  ...
  await p.locator('[data-view="library"]').click();await p.locator('.conversation-document:visible').first().click();await eventually(async()=>await p.locator('.library-prose:visible').count()===3);assert.equal(await p.locator('#document-panel').getByText('智能过滤内容',{exact:true}).count(),0);assert.equal(await p.locator('#filter-onboarding:visible').count(),0);
  ...
  await p.locator('#back').click();await p.locator('#search').fill('');await p.locator('[data-view="settings"]').click();await p.locator('[name="smart-filter-mode"][value="light"]').waitFor();await eventually(async()=>await p.locator('[value="light"]').isChecked(),'Settings must render the persisted Light mode');assert.equal(await p.locator('[value="light"]').isChecked(),true);await p.locator('#filter-recent-open').click();await p.locator('.filter-recent-row').first().waitFor();await p.screenshot({path:'work/smart-filter-recent-synthetic.png',fullPage:true});await p.locator('.filter-recent-row').first().getByRole('button',{name:'恢复到 Input Archive'}).click();await eventually(async()=>(await p.evaluate(()=>chrome.runtime.sendMessage({type:'FILTER_RECENT'}))).data.items.length===1);await p.locator('#filter-recent-close').click();await p.locator('[value="off"]').check();await p.locator('[data-view="library"]').click();await p.locator('.conversation-document:visible').first().click();await eventually(async()=>await p.locator('.library-prose:visible').count()===5);
  await p.locator('[data-view="settings"]').click();await p.locator('[value="light"]').check();await p.locator('[data-view="library"]').click();await p.locator('.conversation-document:visible').first().click();await eventually(async()=>await p.locator('.library-prose:visible').count()===4);
...
test('first input event protects even when immediate Undo leaves no changed text to autosave',{timeout:60000},async()=>{
 const h=await FakeChatGPT.start();try{const p=h.archive;await p.locator('#consent-check').check();await p.locator('#enable-consent').click();const c=conversation('instant-undo-synthetic');c.messages=[{id:'instant-undo-source',text:'继续'}];const chat=await h.open(c,{arrival:'empty'});await chat.evaluate(c=>{window.fake.render(c);const root=document.querySelector('#messages > div'),article=document.createElement('article');article.dataset.testid='conversation-turn-0';root.before(article);article.append(root);},c);await eventually(async()=>(await p.evaluate(()=>chrome.runtime.sendMessage({type:'FILTER_RECENT'}))).data.items.length===1);
  const before=await h.state();await p.locator('[data-view="settings"]').click();await p.locator('[value="off"]').check();await p.locator('[data-view="library"]').click();await p.locator('.conversation-document:visible').first().click();const field=p.locator('.library-prose');await field.fill('Synthetic brief edit');await field.press(process.platform==='darwin'?'Meta+z':'Control+z');await eventually(async()=>await field.textContent()==='继续');await eventually(async()=>(await p.evaluate(()=>chrome.runtime.sendMessage({type:'FILTER_RECENT'}))).data.items.length===0);await p.locator('[data-view="settings"]').click();await p.locator('[value="light"]').check();await p.locator('[data-view="library"]').click();await p.locator('.conversation-document:visible').first().click();await field.waitFor();assert.equal(await field.textContent(),'继续');assert.deepEqual((await h.state()).records,before.records);assert.equal((await h.state()).library.blocks[0].libraryText,null);assert.equal(h.extensionNetworkRequests,0);assert.deepEqual(h.errors,[]);
 }finally{await h.close();}
});
