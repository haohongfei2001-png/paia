...
test('Smart Filter isolated Chrome: capture, Light reading, full search/context, keep, Off and no document controls',{timeout:90000},async()=>{
 const h=await FakeChatGPT.start({headless:false});try{
  ...
  await p.locator('[data-view="library"]').click();await openCapturedReader(p);await eventually(async()=>await p.locator('.library-prose:visible').count()===3);assert.equal(await p.locator('#document-panel').getByText('智能过滤内容',{exact:true}).count(),0);assert.equal(await p.locator('#filter-onboarding:visible').count(),0);
  ...
  await p.locator('#back').click();await p.locator('#search').fill('');await p.locator('[data-view="settings"]').click();await p.locator('[name="smart-filter-mode"][value="light"]').waitFor();await eventually(async()=>await p.locator('[value="light"]').isChecked(),'Settings must render the persisted Light mode');assert.equal(await p.locator('[value="light"]').isChecked(),true);await p.locator('#filter-recent-open').click();await p.locator('.filter-recent-row').first().waitFor();await p.screenshot({path:'work/smart-filter-recent-synthetic.png',fullPage:true});await p.locator('.filter-recent-row').first().getByRole('button',{name:'恢复到 Input Archive'}).click();await eventually(async()=>(await p.evaluate(()=>chrome.runtime.sendMessage({type:'FILTER_RECENT'}))).data.items.length===1);await p.locator('#filter-recent-close').click();await p.locator('[value="off"]').check();await p.locator('[data-view="library"]').click();await openCapturedReader(p);await eventually(async()=>await p.locator('.library-prose:visible').count()===5);
  await p.locator('[data-view="settings"]').click();await p.locator('[value="light"]').check();await p.locator('[data-view="library"]').click();await openCapturedReader(p);await eventually(async()=>await p.locator('.library-prose:visible').count()===4);
