...
test('Smart Filter isolated Chrome: capture, Light reading, full search/context, keep, Off and no document controls',{timeout:90000},async()=>{
 const h=await FakeChatGPT.start();try{
  ...
  await p.locator('.conversation-document').click();await eventually(async()=>await p.locator('.library-prose:visible').count()===3);assert.equal(await p.locator('#document-panel').getByText('智能过滤内容',{exact:true}).count(),0);assert.equal(await p.locator('#filter-onboarding:visible').count(),0);
  await p.locator('#back').click();await p.locator('#search').fill('请继续');await p.locator('.search-input').waitFor();assert.equal(await p.locator('.filter-search-label').textContent(),'智能过滤内容');await mkdir('work',{recursive:true});await p.screenshot({path:'work/smart-filter-search-synthetic.png',fullPage:true});await p.locator('.search-input').click();await eventually(async()=>(await p.locator('.library-prose').allTextContents()).includes('请继续'));
  assert.equal((await p.evaluate(()=>chrome.runtime.sendMessage({type:'FILTER_RECENT'}))).data.items.length,2);
  await p.locator('#back').click();await p.locator('#search').fill('');await p.locator('[data-view="settings"]').click();await p.locator('[name="smart-filter-mode"][value="light"]').waitFor();await eventually(async()=>await p.locator('[value="light"]').isChecked(),'Settings must render the persisted Light mode');assert.equal(await p.locator('[value="light"]').isChecked(),true);await p.locator('#filter-recent-open').click();await p.locator('.filter-recent-row').first().waitFor();await p.screenshot({path:'work/smart-filter-recent-synthetic.png',fullPage:true});await p.locator('.filter-recent-row').first().getByRole('button',{name:'恢复到 Input Archive'}).click();await eventually(async()=>(await p.evaluate(()=>chrome.runtime.sendMessage({type:'FILTER_RECENT'}))).data.items.length===1);await p.locator('#filter-recent-close').click();await p.locator('[value="off"]').check();await p.locator('[data-view="library"]').click();await p.locator('.conversation-document').click();await eventually(async()=>await p.locator('.library-prose:visible').count()===5);
  await p.locator('[data-view="settings"]').click();await p.locator('[value="light"]').check();await p.locator('[data-view="library"]').click();await p.locator('.conversation-document').click();await eventually(async()=>await p.locator('.library-prose:visible').count()===4);
  ...
  await p.locator('.input-remove').first().click();await eventually(async()=>(await h.state()).library.blocks.some(b=>b.excluded));await p.locator('#notice').getByRole('button',{name:'撤销',exact:true}).click();await eventually(async()=>(await h.state()).library.blocks.every(b=>!b.excluded));assert.deepEqual((await h.state()).records,baseline);
