...
test('formal backup round trip preserves working text, provenance, human edits and important revisions',async()=>{
 const f=await completeFixture();await f.runner.wake({userActionId:op()});const e=(await rows(f.s,'thoughts'))[0];await f.s.editEntry({id:e.id,expectedRevision:e.revision,expectedInputRevision:(await f.s.entry(e.id)).currentInputRevision,changes:{body:e.thoughtText+' 人工维护的独立内容'},operationId:op()});await f.s.setOrganizerControls({readingSort:'desc',libraryView:'ai'});await f.s.drainLibraryMaintenance();const items=await exported(new BackupService(f.s,{appVersion:'0.8.1'}));assert.equal(items[0].format,'PAIA Backup');assert.doesNotMatch(JSON.stringify(items),/synthetic-test-key|apiKey|deepseek-api-key|recordIndex|blockIndex/);const validator=new BackupValidator();for(const item of items)await validator.add(item);assert.equal(validator.preview().counts.entries,5);
...
test('empty-library restore refuses an existing document without source records',async()=>{
 ...
 await target.s.repository.transaction(true,t=>t.put('documents',{
  id:'local-unsourced-document',value:{id:'local-unsourced-document',platform:'chatgpt',
   sourceConversationId:'local-unsourced-chat',userTitle:'Local draft',status:'active'},
 }));
 ...
 assert.equal((await rows(target.s,'documents')).length,1);
