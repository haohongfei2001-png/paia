...
import {invalidateThoughtTopicIndex} from '../core/thought-read-index.js';
...
test('300 Entry Topic uses bounded continuous descriptor chunks, domain section seek and changed-time cursor fence',async()=>{
 ...
 assert.equal(ids.length,300);assert.equal(new Set(ids).size,300);assert.equal(pages.length,8);const jump=await f.s.topicDocumentPage({topicId:topic.id,sort:'asc',sectionId:sections[2]});assert.equal(jump.items[0].placement.sectionId,sections[2]);
 await f.s.foundationWrite(async t=>{const e=await t.get('thoughts',ids[0]);await t.put('thoughts',{...e,createdAt:'2099-01-01T00:00:00Z'});await invalidateThoughtTopicIndex(f.s,t,topic.id,{sourceTime:true});});assert.equal((await f.s.topicDocumentPage({topicId:topic.id,sort:'asc',cursor:first.nextCursor})).cursorInvalid,true);assert.equal(f.requests.length,0);assert.ok(performance.now()-start<10000);
});
