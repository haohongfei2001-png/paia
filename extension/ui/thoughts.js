...
export class ThoughtWorkspace extends BaseThoughtWorkspace{
 ...
 rememberView(){if(this.id){const anchor=this.view==='original'?this.topicAnchor():null;this.aiViewSession.remember(this.id,this.view,{scroll:Math.max(0,scrollY||0),cursor:this.cursor,pages:this.pages,query:$('topic-search').value||'',anchor});}}
 ...
 async switchView(view,{restoreFocus=null}={}){
  if(!['original','ai'].includes(view))return;if(!this.id){this.requestedTopicView=view;return;}if(this.viewSwitchPromise){await this.viewSwitchPromise;return this.switchView(view,{restoreFocus});}if(view===this.view)return;
 ...
 async open(id){
  ...
  this.homePositions.set(this.id||'home',{scroll:scrollY,cursor:this.cursor,pages:this.pages,query:this.id?$('topic-search').value:$('thought-search').value,sort:this.readingSort,...(currentAnchor?{anchor:currentAnchor}:{})});
  ...
  this.id=id;$('ai-presentation-toggle').checked=this.view==='ai';$('ai-presentation-toggle').disabled=!!this.pendingView;$('thought-list').classList.toggle('ai-mode',this.view==='ai');if(this.view!=='ai')$('ai-library-update').hidden=true;this.onOpen();
  ...
  $('topic-search').value=id?(session?.query??saved?.query??''):'';if(!id&&saved)$('thought-search').value=saved.query;this.cursor=session?.cursor??saved?.cursor??null;this.pages=session?.pages||saved?.pages||[];this.history=null;if(sessionAnchor)this.topicNavigationAnchor={entryId:sessionAnchor.id,top:sessionAnchor.top};
  await this.refresh();if(intent!==this.openIntent)return;this.onOpen();
