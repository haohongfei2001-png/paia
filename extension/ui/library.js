...
export class DocumentEditor {
 constructor(root,state,doc,onStatus,onChange){
  this.root=root;root.tabIndex=-1;this.id=doc.id;this.onStatus=onStatus;this.onChange=onChange;this.savedTitle=doc.userTitle;this.title=doc.userTitle;this.titleRevision=doc.titleRevision;this.originalTitle=doc.originalConversationTitle;this.entries=new Map();this.journal=new UndoJournal();this.revisions=new RevisionSession();this.autosave=new AutosaveSession(()=>void this.flush(),{delay:500});this.undoStack=[];this.redoStack=[];this.composing=false;this.saving=false;this.failed=false;this.disposed=false;
  for(const b of state.library.blocks.filter(b=>b.documentId===doc.id))this.entries.set(b.id,{saved:value(b),local:value(b),revision:b.revision,signature:b.provenanceSignature,original:state.records.find(r=>r.id===b.originalTextReference)?.originalText??'',lastNonempty:value(b)});
 ...
 change(changes,record=true){const patches=[];for(const {id,after}of changes){const before=id==='title'?this.title:{...this.entries.get(id).local};if(equal(before,after))continue;patches.push({id,before,after});if(id==='title')this.title=after;else{const e=this.entries.get(id);e.local={...after};if(this.text(e).length)e.lastNonempty={...e.local};}}
  if(!patches.length)return;if(record){this.journal.record(patches);}this.failed=false;if(!this.conflicted)this.onStatus('正在保存…');this.autosave.schedule();this.onChange();
 }
 ...
 async flush(){this.autosave.cancel();if(this.disposed||this.composing||this.failed||this.conflicted)return false;if(this.saving){await this.pending;return this.dirty()?this.flush():true;}if(!this.dirty())return true;
  const blocks=[...this.entries].filter(([,e])=>!equal(e.local,e.saved)).map(([id,e])=>({id,expectedRevision:e.revision,...e.local}));const title=this.title!==this.savedTitle?this.title:undefined;this.saving=true;
  const edit={documentId:this.id,blocks,...(title===undefined?{}:{title,expectedTitleRevision:this.titleRevision})};edit.operationId=this.revisions.attempt(edit).operationId;
  this.pending=(async()=>{try{const result=await request('EDIT_DOCUMENT',{edit});if(result.conflict){this.failed=true;this.conflicted=true;this.onStatus('其他页面或来源发生变化。当前修改尚未保存。','conflict');return false;}for(const b of blocks){const e=this.entries.get(b.id);e.saved={libraryText:b.libraryText,note:b.note,excluded:b.excluded};e.revision++;}if(title!==undefined){this.savedTitle=title;this.titleRevision++;}this.onStatus(this.dirty()?'正在保存…':'已保存到本机');return true;}catch{this.failed=true;this.onStatus('尚未保存，内容仍在这里。','error');return false;}finally{this.saving=false;if(this.deferredState){const incoming=this.deferredState;this.deferredState=null;this.deferredEffect=this.receive(incoming);}}})();const ok=await this.pending;if(ok&&this.dirty())return this.flush();return ok;
 }
