...
export class DocumentEditor {
 constructor(root,state,doc,onStatus,onChange,onRecovered=()=>{}){
  ...
  this.surface=new PlainTextSurface(root,{start:()=>{this.composing=true;this.autosave.cancel();},end:()=>{this.composing=false;this.collect();if(this.deferredState){const incoming=this.deferredState;this.deferredState=null;this.deferredEffect=this.receive(incoming);}},input:()=>this.collect(),leave:()=>{this.collect();void this.flush();}});
 ...
 collect(){if(this.composing||this.disposed)return;const changes=[];const title=this.root.querySelector('#document-title');const entered=title.innerText.replace(/\n/g,'').slice(0,300);const desired=entered===(this.originalTitle||'独立整理文档')&&this.title===''?'':entered;if(desired!==this.title)changes.push({id:'title',after:desired});for(const [id,e]of this.entries){const el=this.field(id);if(!el||e.local.excluded)continue;const text=el.textContent===''?'':el.innerText;if(text!==this.text(e))changes.push({id,after:{...e.local,libraryText:text}});}if(changes.length)this.change(changes);}
 change(changes,record=true){const patches=[];for(const {id,after}of changes){const before=id==='title'?this.title:{...this.entries.get(id).local};if(equal(before,after))continue;patches.push({id,before,after});if(id==='title')this.title=after;else{const e=this.entries.get(id);e.local={...after};if(this.text(e).length)e.lastNonempty={...e.local};}}
  if(!patches.length)return;if(record){this.journal.record(patches);}this.failed=false;if(!this.conflicted)this.onStatus('正在保存…');this.autosave.schedule();void this.protectRecovery();this.onChange();
 }
 ...
 exclude(id,excluded){const e=this.entries.get(id);if(!e)return;this.change([{id,after:{...e.local,excluded}}]);this.paint();this.root.focus();void this.flush();}
 note(id,text){const e=this.entries.get(id);if(e)this.change([{id,after:{...e.local,note:text}}]);}
 paint(){const title=this.root.querySelector('#document-title');if(title&&title.innerText!==(this.title||this.originalTitle||'独立整理文档'))title.textContent=this.title||this.originalTitle||'独立整理文档';for(const [id,e]of this.entries){const el=this.field(id);if(el){if(el.innerText!==this.text(e))el.textContent=this.text(e);el.closest('.library-block').hidden=e.local.excluded;}}this.onChange();}
 ...
 buildEdit(){if(!this.dirty())return null;const blocks=[...this.entries].filter(([,e])=>!equal(e.local,e.saved)).map(([id,e])=>({id,expectedRevision:e.revision,...e.local})),title=this.title!==this.savedTitle?this.title:undefined,base={documentId:this.id,blocks,...(title===undefined?{}:{title,expectedTitleRevision:this.titleRevision})};return this.revisions.attempt(base);}
 async protectRecovery(edit=this.buildEdit()){if(!edit)return true;try{await this.recovery.protect({type:'EDIT_DOCUMENT',edit},edit.operationId);this.recoveryFailed=false;return true;}catch{this.recoveryFailed=true;return false;}}
 ...
 async restoreRecovery(){let draft;try{draft=await this.recovery.load();}catch{return false;}if(!draft)return false;const operation=draft.operation,edit=operation?.edit;if(operation?.type!=='EDIT_DOCUMENT'||edit?.documentId!==this.id){await this.recovery.clear(draft.token).catch(()=>{});return false;}try{const result=await request('EDIT_DOCUMENT',{edit});if(result?.conflict){this.applyRecovery(edit);this.failed=true;this.conflicted=true;this.onStatus('检测到上次未完成的修改，但已保存版本同时发生变化。草稿已恢复到页面，未自动覆盖。','conflict');return false;}await this.recovery.clear(draft.token).catch(()=>{});this.onStatus('已恢复上次未完成的修改');queueMicrotask(()=>this.onRecovered());return true;}catch(error){if(error?.code==='INVALID_REQUEST'){await this.recovery.clear(draft.token).catch(()=>{});return false;}this.applyRecovery(edit);this.failed=true;this.onStatus('检测到上次未完成的修改。草稿仍保存在本机，可在连接恢复后重试。','error');return false;}}
 get recoveryPending(){return !!this.recovery?.pending;}
 async flush(){this.autosave.cancel();if(this.disposed||this.composing||this.failed||this.conflicted)return false;if(this.saving){await this.pending;return this.dirty()?this.flush():true;}if(!this.dirty())return true;
  const edit=this.buildEdit(),blocks=edit.blocks,title=edit.title;this.saving=true;
  this.pending=(async()=>{await this.protectRecovery(edit);try{const result=await request('EDIT_DOCUMENT',{edit});if(result.conflict){this.failed=true;this.conflicted=true;this.onStatus('其他页面或来源发生变化。当前修改尚未保存。','conflict');return false;}for(const b of blocks){const e=this.entries.get(b.id);e.saved={libraryText:b.libraryText,note:b.note,excluded:b.excluded};e.revision++;}if(title!==undefined){this.savedTitle=title;this.titleRevision++;}this.onStatus(this.dirty()?'正在保存…':'已保存到本机');void this.recovery.clear(edit.operationId);return true;}catch{this.failed=true;this.onStatus(this.recoveryFailed?'尚未保存；恢复草稿也未能写入。请保持此页打开并重试或复制文字。':'尚未保存，内容仍在这里；恢复草稿已保存在本机。','error');return false;}finally{this.saving=false;if(this.deferredState){const incoming=this.deferredState;this.deferredState=null;this.deferredEffect=this.receive(incoming);}}})();const ok=await this.pending;if(ok&&this.dirty())return this.flush();return ok;
 }
 receive(state){if(this.saving||this.composing){this.deferredState=state;return;}const doc=state.conversations.find(d=>d.id===this.id);let invalid=false;for(const [id,e]of this.entries){const b=state.library.blocks.find(b=>b.id===id);if(!b||b.provenanceSignature!==e.signature){invalid=true;break;}}
  ...
  for(const [id,e]of this.entries){const b=state.library.blocks.find(b=>b.id===id);if(b.revision>e.revision){if(!equal(e.local,e.saved)||document.activeElement===this.field(id)){this.failed=true;this.conflicted=true;this.onStatus('其他页面已修改正文，当前修改尚未保存。','conflict');}else{e.local=e.saved=value(b);e.revision=b.revision;const el=this.field(id);if(el){el.textContent=this.text(e);el.closest('.library-block').hidden=e.local.excluded;}this.undoStack=[];this.redoStack=[];}}}if(!this.dirty()&&!this.root.contains(document.activeElement))this.paint();return newContent?{newContent:true}:undefined;
 }
 ...
 dispose(){this.disposed=true;this.autosave.dispose();this.surface.dispose();this.controller.abort();this.undoStack=[];this.redoStack=[];this.entries.clear();}
}
