...
export class DocumentEditor {
 ...
 receive(state){if(this.saving){this.deferredState=state;return;}const doc=state.conversations.find(d=>d.id===this.id);let invalid=false;for(const [id,e]of this.entries){const b=state.library.blocks.find(b=>b.id===id);if(!b||b.provenanceSignature!==e.signature){invalid=true;break;}}
  if(invalid){this.autosave.cancel();this.undoStack=[];this.redoStack=[];const carry=[];let conflict=false;for(const [id,e]of this.entries){const b=state.library.blocks.find(b=>b.id===id);if(b&&b.provenanceSignature===e.signature&&!equal(e.local,e.saved)){carry.push({id,after:{...e.local}});if(b.revision!==e.revision)conflict=true;}}if(doc&&this.title!==this.savedTitle){carry.push({id:'title',after:this.title});if(doc.titleRevision!==this.titleRevision)conflict=true;}return {rebuild:true,carry,conflict,removed:true};}
