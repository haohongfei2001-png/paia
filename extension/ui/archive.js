...
$('document-filter-toggle').addEventListener('click',()=>{if(view==='library'&&documentId)void navigate('library',documentId,null,{showFiltered:!showFilteredCurrent,resetReadingPosition:true});});
...
async function readDocumentSearchPage(id,searchQuery,cursor,intent){
 ...
 for(;;){const page=await request('SEARCH_INPUTS',{options:{universal:true,paged:true,query:searchQuery,mode:'current',documentId:id,topicId:null,source:'',dateFrom:'',to:'',types:['input'],includeRemoved:false,includeFiltered:$('document-search-include-filtered').checked,cursor:at,limit:40}});if(intent!==documentSearchIntent||id!==documentId||view!=='library')return null;if(page.items.length||!page.nextCursor)return page;const before=JSON.stringify(at),after=JSON.stringify(page.nextCursor);if(before===after||++guard>50)throw Error('INVALID_SEARCH_CURSOR');at=page.nextCursor;}
}
...
$('document-search-retry').addEventListener('click',()=>{void runDocumentSearch();});$('document-search-include-filtered').addEventListener('change',()=>{if(view==='library'&&documentId)void runDocumentSearch({reset:true});});
...
function appendDocumentPage(page){const doc=page.conversations.find(d=>d.id===documentId);if(!doc)return null;const editable=view==='library',rows=view==='archive'?archiveRows(page,doc.id):documentBlocks(page,doc.id,view==='excluded'),pageBody=element('div','reader-page'),body=$('document-body');let previous=null;
 ...
 const stamp=element('p','block-time',time(b.sourceSentAt));stamp.hidden=false;section.append(stamp);if(editable&&page.filteredPageItemIds?.includes(b.id))section.append(element('p','filtered-input-note','智能过滤曾收起这条输入 · 内容未删除'));
...
function renderDocument(){$('reader-window-guard').hidden=true;$('document-page').inert=false;const doc=state.conversations.find(d=>d.id===documentId);if(!doc){documentId=null;render();return;}
 ...
 const filterToggle=$('document-filter-toggle');filterToggle.hidden=!editable;if(editable){filterToggle.setAttribute('aria-pressed',String(showFilteredCurrent));filterToggle.textContent=showFilteredCurrent?'只显示未收起的输入':'显示本聊天全部输入';}
 ...
 $('document-page').classList.toggle('standard',state.preferences.timeEmphasis==='standard');if(editable){editor=new DocumentEditor($('document-page'),state,doc,status,counts,()=>{notify('已恢复上次未完成的修改。');void refresh();});if(partHistory){editor.importHistory(partHistory);partHistory=null;}readerStream={documentId,sort:state.readingSort,snapshot:state.readingSnapshot,includeFiltered:showFilteredCurrent,cursors:[pageCursor],first:0,last:0,highWater:0,endCursor:state.nextCursor,loading:false};reader.mount();requestAnimationFrame(()=>{if(readerStream?.documentId===documentId&&$('document-body').getBoundingClientRect().bottom<innerHeight+700)void loadReaderPage('forward');});}else status(view==='archive'?'原文只读':'已移除 · 可恢复显示');renderDocumentSearch();
}
