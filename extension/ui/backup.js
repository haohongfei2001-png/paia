...
export async function* backupSegmentRows(files){
 const list=Array.from(files||[]),manifests=list.filter(file=>file.name.endsWith('.manifest.json'));
 if(manifests.length!==1||list.length<2)backupError('BACKUP_INCOMPLETE');
 const manifestFile=manifests[0],prefix=manifestFile.name.slice(0,-'.manifest.json'.length);
