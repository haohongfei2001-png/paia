...
export class BackupPanel {
 ...
 async cancel(){if(this.busy)return;if(this.sessionId)await request('PAIA_BACKUP_CANCEL',{options:{sessionId:this.sessionId}}).catch(()=>{});this.sessionId=null;this.preview=null;$('backup-restore').disabled=true;this.mode='empty';$('backup-mode').value='empty';$('backup-confirm-replace').checked=false;$('backup-preview').hidden=true;$('backup-file').value='';this.status('');}
 async create(){
  if(this.busy)return;
 ...
 async createSegmented(){
  ...
  try{
   ...
   downloadParts([JSON.stringify(manifest)],name+'.manifest.paia-backup','application/json');
