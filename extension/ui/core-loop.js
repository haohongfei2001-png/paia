...
function refreshHome(){const home=$('core-loop-home'),browse=$('core-loop-browse-title')?.parentElement;if(!home)return;const visible=isArchiveHome();home.hidden=!visible;if(browse)browse.hidden=!visible;if(!visible)return;const docs=hasArchiveDocuments();if(docs){if(!home.dataset.state||home.dataset.state==='activation-empty')setActivationReady();}else setActivationEmpty();void refreshRecent();void refreshReading();void refreshReturnCard();}
