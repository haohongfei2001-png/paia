...
const definitions=Object.freeze({
 ...
 revisit_open:Object.freeze({state:['first','new','quiet']}),
 revisit_item_open:Object.freeze({kind:['new_input','topic','old_input']}),
 revisit_mark:Object.freeze({}),
 ...
 core_loop_action:Object.freeze({action:['continue','find','return','reuse']})
});
...
function activeDaysFor(row,name,days=null,now=Date.now()){if(!plain(row?.daily))return 0;const cutoff=days===null?dayKey(now-(PRODUCT_SIGNAL_RETENTION_DAYS-1)*86400000):dayKey(now-(days-1)*86400000);return Object.entries(row.daily).filter(([day,bucket])=>day>=cutoff&&plain(bucket)&&Object.keys(bucket).some(key=>(key===name||key.startsWith(name+'|'))&&count(bucket[key])>0)).length;}
...
export function summarizeProductSignals(row,now=Date.now()){
 ...
 const build=(counts,days)=>{
  ...
  const revisitOpen=sum(counts,'revisit_open'),revisitItems=sum(counts,'revisit_item_open'),revisitMarks=sum(counts,'revisit_mark');
  ...
  const coreContinue=get(counts,'core_loop_action',{action:'continue'}),coreFind=get(counts,'core_loop_action',{action:'find'}),coreReturn=get(counts,'core_loop_action',{action:'return'}),coreReuse=get(counts,'core_loop_action',{action:'reuse'});
  return {input:{searches:inputSearch,searchHits:inputHits,searchHitRate:ratio(inputHits,inputSearch),targetedOpens:inputTarget,searchOpens:inputSearchOpen,searchCopies:inputSearchCopies,copies:inputCopies,old180DayOpens:inputOld180,searchCopyPerOpen:ratio(inputSearchCopies,inputSearchOpen)},thought:{searches:thoughtSearch,searchHits:thoughtHits,searchHitRate:ratio(thoughtHits,thoughtSearch),searchOpens:thoughtSearchOpen,topicOpens:topicOpen,repeatTopicOpens:topicRepeat,repeatOpenShare:ratio(topicRepeat,topicOpen),copies:thoughtCopies,aiViewOpens:aiOpen,returnsToOriginal:aiOriginal,aiSavedEdits:aiEdits},universal:{searches:universalSearch,searchHits:universalHits,searchHitRate:ratio(universalHits,universalSearch),resultOpens:universalOpen,contextPrepares:universalPrepare,openPerSearch:ratio(universalOpen,universalSearch),preparePerSearch:ratio(universalPrepare,universalSearch),opensByKind:{input:get(counts,'universal_result_open',{kind:'input'}),thought:get(counts,'universal_result_open',{kind:'thought'}),ai:get(counts,'universal_result_open',{kind:'ai'})},preparesByKind:{input:get(counts,'context_prepare_from_search',{kind:'input'}),thought:get(counts,'context_prepare_from_search',{kind:'thought'}),ai:get(counts,'context_prepare_from_search',{kind:'ai'})}},revisit:{opens:revisitOpen,opensWithNew:get(counts,'revisit_open',{state:'new'}),quietOpens:get(counts,'revisit_open',{state:'quiet'}),firstOpens:get(counts,'revisit_open',{state:'first'}),itemOpens:revisitItems,marks:revisitMarks,itemOpenPerVisit:ratio(revisitItems,revisitOpen),itemsByKind:{newInput:get(counts,'revisit_item_open',{kind:'new_input'}),topic:get(counts,'revisit_item_open',{kind:'topic'}),oldInput:get(counts,'revisit_item_open',{kind:'old_input'})}},context:{builds:contextBuild,nonEmptyBuilds:contextHit,buildHitRate:ratio(contextHit,contextBuild),customProfileBuilds:contextCustom,copies:contextCopy,markdownExports:contextMarkdown,shares:contextShare,sharePerBuild:ratio(contextShare,contextBuild)},coreLoop:{continueReads:coreContinue,finds:coreFind,returns:coreReturn,reuses:coreReuse,total:coreContinue+coreFind+coreReturn+coreReuse,activeDays:activeDaysFor(row,'core_loop_action',days,now)}};
 };
 return {version:PRODUCT_SIGNAL_VERSION,enabled:row?.enabled===true,localOnly:true,containsContent:false,retentionDays:PRODUCT_SIGNAL_RETENTION_DAYS,createdAt:row?.createdAt||null,updatedAt:row?.updatedAt||null,all:build(all,null),last30Days:build(recent,30),activeDaysLast30:daily.filter(x=>x.total>0).length,daily};
}
...
export class ProductSignals {
 ...
 async observe(request,result,sender){
  switch(request?.type){
   ...
   case 'PAIA_CORE_LOOP_ACTION': return this.record({name:'core_loop_action',dimensions:{action:request.action}});
  }
 }
}
