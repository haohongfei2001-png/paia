...
export function validatePassportRow(row){
 ...
 if(row.kind==='grant'){
  const allowed=['id','kind','version','grantId','consumer','purpose','profileId','permission','duration','createdAt','expiresAt','revokedAt','consumedAt','lastUsedAt','useCount'];
  if(Object.keys(row).some(k=>!allowed.includes(k))||row.id!==passportGrantId(row.grantId)||!idOK(row.grantId)||!CONTEXT_CONSUMERS.includes(row.consumer)||row.consumer==='manual'||!CONTEXT_PURPOSES.includes(row.purpose)||row.purpose==='current_task'||!idOK(row.profileId)||row.permission!==PASSPORT_PERMISSION||!PASSPORT_DURATIONS.includes(row.duration)||!Number.isFinite(Date.parse(row.createdAt))||row.expiresAt!==null&&!Number.isFinite(Date.parse(row.expiresAt))||row.revokedAt!==null&&!Number.isFinite(Date.parse(row.revokedAt))||row.consumedAt!==null&&!Number.isFinite(Date.parse(row.consumedAt))||row.lastUsedAt!==null&&!Number.isFinite(Date.parse(row.lastUsedAt))||!Number.isSafeInteger(row.useCount)||row.useCount<0)return false;
  if(row.duration==='once'&&row.expiresAt!==null)return false;
  if(row.duration!=='once'&&row.expiresAt===null)return false;
  return true;
 }
 if(row.kind==='audit'){
  const allowed=['id','kind','version','auditId','grantId','consumer','purpose','profileId','permission','action','createdAt'];
  return !Object.keys(row).some(k=>!allowed.includes(k))&&row.id===passportAuditId(row.createdAt,row.auditId)&&idOK(row.auditId)&&(row.grantId===null||idOK(row.grantId))&&CONTEXT_CONSUMERS.includes(row.consumer)&&CONTEXT_PURPOSES.includes(row.purpose)&&idOK(row.profileId)&&row.permission===PASSPORT_PERMISSION&&['copy','markdown','manual_copy','manual_markdown'].includes(row.action)&&Number.isFinite(Date.parse(row.createdAt));
 }
 return false;
}
