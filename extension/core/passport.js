...
export class PassportService {
 ...
 async authorize({grantId,consumer,purpose,profileId}={}){if(!idOK(grantId)||!CONTEXT_CONSUMERS.includes(consumer)||!CONTEXT_PURPOSES.includes(purpose)||!idOK(profileId))invalid();const row=await this.resolve(grantId,{profileId});if(row.consumer!==consumer||row.purpose!==purpose)throw new ArchiveError('MEMORY_DENIED');return row;}
