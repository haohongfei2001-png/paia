import {assertLocalNetworkAllowed} from './local-network-policy.js';
...
export class PassportService {
 ...
 async resolve(grantId,{profileId}={}){
  if(!idOK(grantId)||profileId!==undefined&&!idOK(profileId))invalid();
  return this.run(false,async t=>{const row=await t.get('meta',passportGrantId(grantId));if(!validatePassportRow(row)||row.kind!=='grant'||grantState(row,this.clock())!=='active'||row.resourceScope!==PASSPORT_RESOURCE_SCOPE||row.permission!==PASSPORT_PERMISSION||profileId!==undefined&&row.profileId!==profileId)throw new ArchiveError('MEMORY_DENIED');if(!await t.get('meta',memoryKey('profile',row.profileId)))throw new ArchiveError('MEMORY_UNAVAILABLE');return row;});
 }
 async authorize({grantId,consumer,purpose,profileId}={}){if(!idOK(grantId)||!CONTEXT_CONSUMERS.includes(consumer)||!CONTEXT_PURPOSES.includes(purpose)||!idOK(profileId))invalid();await assertLocalNetworkAllowed(this.store);const row=await this.resolve(grantId,{profileId});if(row.consumer!==consumer||row.purpose!==purpose)throw new ArchiveError('MEMORY_DENIED');return row;}
