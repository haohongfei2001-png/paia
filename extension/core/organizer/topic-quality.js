...
const candidateText=candidate=>[candidate?.name,...(candidate?.sections||[]).map(section=>section?.name).filter(name=>name&&!isGenericSectionName(name))].filter(Boolean).join(' ');
const strongCandidateMatch=(inputText,candidate)=>{const candidateName=normalized(candidate?.name);const input=normalized(inputText);if(candidateName&&candidateName.length>=4&&input.includes(candidateName))return true;return tokenStats(inputText,candidateText(candidate)).weighted>=3;};
