...
export function validateAIPresentation(output,request){
 ...
 for(const field of AI_LIST_FIELDS){result[field]=[];if(!Array.isArray(output[field]))continue;for(const row of output[field]){if(!record(row)||typeof row.text!=='string'||!row.text.trim())continue;const ids=evidence(row.evidenceEntryIds);if(!ids.length)continue;result[field].push({text:row.text.trim(),evidenceEntryIds:ids});}}
 ...
 for(const field of ['blockSummary','currentView'])if(roots.length&&typeof output[field]==='string')result[field]=output[field].trim();
