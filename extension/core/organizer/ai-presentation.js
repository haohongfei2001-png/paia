...
import {createAIPresentationCandidate,publicAIPresentationCandidate,applyAIPresentationCandidateDecision} from './ai-candidate.js';
...
async function snapshot(s){await migrateAIPresentations(s);return s.run(()=>s.repository.transaction(false,async t=>{
 ...
 for(const raw of await t.all('topics')){if(raw.lifecycle!=='active'||raw.redirectTo)continue;const topic=await s.canonicalTopic(t,raw.id);if(!validTopicGeneration(topic.activeLayoutGeneration))continue;const current=await topicSnapshot(s,t,topic),rawPrior=cp.topicVersions?.[topic.id]||{},stored=await t.get('meta',ROW+topic.id),allowed=new Set(current.entries.map(x=>x.id));
  ...
  const readable=stored&&stored.topicId===topic.id&&isStoredAIPresentation(stored,allowed),presentation=readable?stored:null,candidate=readable?publicAIPresentationCandidate(stored,allowed):null,prior=readable||!current.entries.length?rawPrior:{};
...
export async function editAIPresentation(s,edit={}){
 const {topicId,field,value,expectedRevision,operationId=null,candidateDecision}=edit;
 if(candidateDecision!==undefined){
  if(!idOK(topicId)||!Number.isSafeInteger(expectedRevision)||expectedRevision<0||!candidateDecision||!AI_FIELDS.includes(candidateDecision.field)||!['adopt','keep'].includes(candidateDecision.decision)||!idOK(operationId)||operationId.length<8)reject('INVALID_OUTPUT');
  await migrateAIPresentations(s);const request={id:topicId,expectedRevision,candidateDecision:{field:candidateDecision.field,decision:candidateDecision.decision},operationId,kind:'ai-candidate-decision'},digest=await hashText(JSON.stringify(request));
  return s.foundationWrite(async t=>{const prior=await receipt(t,request,digest);if(prior)return prior;const row=await t.get('meta',ROW+topicId),topic=await s.canonicalTopic(t,topicId),current=await topicSnapshot(s,t,topic),allowed=new Set(current.entries.map(e=>e.id));if(!row||!isStoredAIPresentation(row,allowed))reject('STALE_BASE');const resolved=applyAIPresentationCandidateDecision(row,{...candidateDecision,expectedRevision},allowed,s.clock());await t.put('meta',resolved.next);if(resolved.changed)await aiJournal(s,t,row,resolved.next,'user',operationId);await candidateFence(t,topicId,resolved.next.candidate||null);const result={revision:resolved.revision,candidateComplete:resolved.complete,field:resolved.field,decision:resolved.decision};await saveReceipt(s,t,request,digest,result);return result;});
 }
...
export class AIPresentationRunner {
 ...
 const protectedUpdate=!!topic.presentation&&Object.values(old?.protections||{}).some(Boolean);let committedRevision=old?.revision||0,candidateCreated=false;
 if(protectedUpdate){const candidate=createAIPresentationCandidate(old,result,{createdAt:s.clock()});if(candidate){const next={...old,candidate,needsUpdate:false,stale:false};await t.put('meta',next);await candidateFence(t,topic.id,candidate);candidateCreated=true;}else{if(old?.candidate){const next={...old};delete next.candidate;await t.put('meta',next);}await candidateFence(t,topic.id,null);}}
 ...
 await t.put('meta',{...row,state:'committed',phase:'completed',requestCount:calls,committedItemCount:1,candidateCreated,phaseTimestamps:{...row.phaseTimestamps,completed:s.clock()}});});return {completed:true,requestCount:calls,topicId:topic.id};
