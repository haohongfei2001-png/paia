...
export async function aiPresentationStatus(s){
 try{return await readAIPresentationStatus(s);}catch(error){
  const compatibility=await s.libraryCompatibilityStatus?.().catch(()=>null),compatibilityReadFailure=error?.code==='INVALID_REQUEST'||error?.code==='STORAGE_FAILED'&&error?.dbCategory==='invalid_key_or_index_value';
  if(compatibility?.unresolvedLayouts>0&&compatibilityReadFailure){
   const delta=await planDelta(s,'ai').catch(()=>null),counts=delta?.counts||{addedInput:0,changedInput:0,removedInput:0,affectedTopic:0,affectedEntry:0};
   return {topics:[],pendingTopics:0,nextTopic:null,counts,approximateBytes:0,runtime:null,degraded:{reason:'topic_compatibility_unresolved',unresolvedLayouts:compatibility.unresolvedLayouts}};
  }
  throw error;
 }
}
