...
async function snapshot(s){await migrateAIPresentations(s);return s.run(()=>s.repository.transaction(false,async t=>{
 ...
 for(const raw of await t.all('topics')){if(raw.lifecycle!=='active'||raw.redirectTo)continue;const topic=await s.canonicalTopic(t,raw.id);if(!validTopicGeneration(topic.activeLayoutGeneration))continue;const current=await topicSnapshot(s,t,topic),rawPrior=cp.topicVersions?.[topic.id]||{},stored=await t.get('meta',ROW+topic.id),allowed=new Set(current.entries.map(x=>x.id));
  ...
  const readable=stored&&stored.topicId===topic.id&&isStoredAIPresentation(stored,allowed),presentation=readable?stored:null,candidate=readable?publicAIPresentationCandidate(stored,allowed,current.versions):null,prior=readable||!current.entries.length?rawPrior:{};
...
export async function editAIPresentation(s,edit={}){
 const {topicId,field,value,expectedRevision,operationId=null,candidateDecisions}=edit;
 if(candidateDecisions!==undefined){
  ...
  await migrateAIPresentations(s);const request={id:topicId,expectedRevision,candidateDecisions:structuredClone(candidateDecisions),operationId,kind:'ai-candidate-save'},digest=await hashText(JSON.stringify(request));
  return s.foundationWrite(async t=>{const prior=await receipt(t,request,digest);if(prior)return prior;const row=await t.get('meta',ROW+topicId),topic=await s.canonicalTopic(t,topicId),current=await topicSnapshot(s,t,topic),allowed=new Set(current.entries.map(e=>e.id));if(!row||!isStoredAIPresentation(row,allowed))reject('STALE_BASE');const resolved=applyAIPresentationCandidate(row,{decisions:candidateDecisions,expectedRevision},allowed,current.versions,s.clock());await t.put('meta',resolved.next);if(resolved.changed)await aiJournal(s,t,row,resolved.next,'user',operationId);await candidateFence(t,topicId,null);const result={revision:resolved.revision,adopted:resolved.adopted,kept:resolved.kept};await saveReceipt(s,t,request,digest,result);return result;});
 }
