...
export async function aiPresentationStatus(s){
 try{return await readAIPresentationStatus(s);}catch(error){
  const compatibility=await s.libraryCompatibilityStatus?.().catch(()=>null);
  if(compatibility?.unresolvedLayouts>0&&['INVALID_REQUEST','STORAGE_FAILED'].includes(error?.code)){
