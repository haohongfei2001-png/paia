...
export async function editAIPresentation(s,edit={}){
 ...
 if(candidateDecisions!==undefined){
  ...
  return s.foundationWrite(async t=>{const prior=await receipt(t,request,digest);if(prior)return prior;const row=await t.get('meta',ROW+topicId),topic=await s.canonicalTopic(t,topicId),current=await topicSnapshot(s,t,topic),allowed=new Set(current.entries.map(e=>e.id));if(!row||!isStoredAIPresentation(row,allowed))reject('STALE_BASE');const resolved=applyAIPresentationCandidate(row,{decisions:candidateDecisions,expectedRevision,expectedCandidateKey},allowed,current.versions,s.clock());await t.put('meta',resolved.next);if(resolved.changed)await aiJournal(s,t,row,resolved.next,'user',operationId);await candidateFence(t,topicId,null);const result={revision:resolved.revision,adopted:resolved.adopted,kept:resolved.kept};await saveReceipt(s,t,request,digest,result);return result;});
 }
