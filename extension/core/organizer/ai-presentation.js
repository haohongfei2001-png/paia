...
import {createAIPresentationCandidate,publicAIPresentationCandidate,applyAIPresentationCandidate} from './ai-candidate.js';
...
async function snapshot(s){await migrateAIPresentations(s);return s.run(()=>s.repository.transaction(false,async t=>{
 ...
 for(const raw of await t.all('topics')){if(raw.lifecycle!=='active'||raw.redirectTo)continue;const topic=await s.canonicalTopic(t,raw.id);if(!validTopicGeneration(topic.activeLayoutGeneration))continue;const current=await topicSnapshot(s,t,topic),rawPrior=cp.topicVersions?.[topic.id]||{},stored=await t.get('meta',ROW+topic.id),allowed=new Set(current.entries.map(x=>x.id));
  ...
  const readable=stored&&stored.topicId===topic.id&&isStoredAIPresentation(stored,allowed),presentation=readable?stored:null,candidate=readable?publicAIPresentationCandidate(stored,allowed,current.versions):null,resetCandidate=readable&&!!stored.candidate&&(!candidate||candidate.stale),baseline=stored?.basedOnCheckpoint?.entryVersions,prior=resetCandidate?(baseline&&typeof baseline==='object'&&!Array.isArray(baseline)?baseline:{}):readable||!current.entries.length?rawPrior:{};
  ...
  topics.push({id:topic.id,name:topic.name,organizationRevision:topic.organizationRevision,generation:topic.activeLayoutGeneration,entries:current.entries,versions:current.versions,inputVersions:current.inputVersions,prior,storedRevision:stored?.revision||0,userDraft:!readable?userAIDraft(stored):null,presentation,candidate,changed,removed,pending:changed.length>0||removed.length>0||!!stored&&(!readable||stored.needsUpdate===true),stale:!!stored&&(!readable||stored.needsUpdate===true||changed.length>0||removed.length>0)});
 }
...
export async function editAIPresentation(s,edit={}){
 ...
 if(candidateDecisions!==undefined){
  ...
  return s.foundationWrite(async t=>{const prior=await receipt(t,request,digest);if(prior)return prior;const row=await t.get('meta',ROW+topicId),topic=await s.canonicalTopic(t,topicId),current=await topicSnapshot(s,t,topic),allowed=new Set(current.entries.map(e=>e.id));if(!row||!isStoredAIPresentation(row,allowed))reject('STALE_BASE');const resolved=applyAIPresentationCandidate(row,{decisions:candidateDecisions,expectedRevision,expectedCandidateKey},allowed,current.versions,s.clock()),checkpoint=await t.get('meta',CHECKPOINT);resolved.next.basedOnCheckpoint={entryVersions:structuredClone(checkpoint?.topicVersions?.[topicId]||row.basedOnCheckpoint?.entryVersions||{})};await t.put('meta',resolved.next);if(resolved.changed)await aiJournal(s,t,row,resolved.next,'user',operationId);await candidateFence(t,topicId,null);const result={revision:resolved.revision,adopted:resolved.adopted,kept:resolved.kept};await saveReceipt(s,t,request,digest,result);return result;});
 }
 ...
 if(equal(row[field],value))return {revision:row.revision};const next={...row,[field]:value,revision:row.revision+1,protections:{...row.protections,[field]:true},userEditedAt:s.clock(),needsUpdate:row.candidate?true:row.needsUpdate};await t.put('meta',next);await aiJournal(s,t,row,next,'user',operationId||s.uuid());const result={revision:next.revision};if(operationId)await saveReceipt(s,t,request,digest,result);return result;
 });
}
...
export class AIPresentationRunner {
 ...
 async run(userActionId,topicId=null,boundedActionId=null){const s=this.s,deadlineAt=Date.now()+30000,signal=this.controller.signal;let requestId=null,usageId=null,credential,phase='preparing',calls=0;const check=()=>{if(signal.aborted)reject('CANCELLED');if(Date.now()>=deadlineAt)reject('PROVIDER_TIMEOUT');};
 ...
 // Return the committed result through the transaction boundary; callback locals are not visible outside it.
 const committed=await s.foundationWrite(async t=>{const row=await t.get('meta',REQUEST+requestId),gate=await t.get('meta','gate');if(!row||!ACTIVE.has(row.state)||signal.aborted||!gate?.enabled||gate.epoch!==all.epoch)reject('CANCELLED');const nowTopic=await s.canonicalTopic(t,topic.id);if(nowTopic.organizationRevision!==topic.organizationRevision||nowTopic.activeLayoutGeneration!==topic.generation)reject('STALE_BASE');const now=await topicSnapshot(s,t,nowTopic);if(!equal(now.versions,topic.versions))reject('STALE_BASE');const allowed=new Set(now.entries.map(e=>e.id));if(result.evidenceEntryIds.some(id=>!allowed.has(id)))reject('INVALID_OUTPUT');const old=await t.get('meta',ROW+topic.id);if((old?.revision||0)!==topic.storedRevision)reject('STALE_BASE');
 ...
 let committedRevision=old?.revision||0,candidateCreated=false;
 if(topic.presentation){const candidate=createAIPresentationCandidate(old,result,{createdAt:s.clock(),materialVersions:topic.versions});if(candidate){const next={...old,candidate,needsUpdate:false,stale:false};await t.put('meta',next);await candidateFence(t,topic.id,candidate);candidateCreated=true;}else{if(old?.candidate){const next={...old};delete next.candidate;next.needsUpdate=false;await t.put('meta',next);}await candidateFence(t,topic.id,null);}}
 else{const next={id:ROW+topic.id,...result,schemaVersion:AI_SCHEMA_VERSION,revision:(old?.revision||0)+1,protections:old?.protections||{},updatedAt:s.clock(),basedOnCheckpoint:{entryVersions:versions},stale:false,needsUpdate:false};delete next.candidate;await t.put('meta',next);await candidateFence(t,topic.id,null);await aiJournal(s,t,topic.presentation,next,'ai',requestId);committedRevision=next.revision;}
 ...
 await t.put('operationReceipts',{id:'ai-presentation:'+requestId,namespace:'ai-presentation',ownerId:topic.id,operationSequence:0,createdAt:s.clock(),result:{committed:1,candidateCreated,revision:committedRevision}});
