...
export const CONTEXT_PACKAGE_RESOURCE_SCOPE='profile';
...
export function contextPackageExpired(pkg,now=Date.now()){
 return !pkg||pkg.type!==CONTEXT_PACKAGE_TYPE||pkg.version!==CONTEXT_PACKAGE_VERSION||pkg.resourceScope!==CONTEXT_PACKAGE_RESOURCE_SCOPE||!Number.isFinite(Date.parse(pkg.expiresAt))||Date.parse(pkg.expiresAt)<=now;
}

export function contextPackageEnvelope(pkg,text,{format='plain',generation=pkg?.generation}={}){
 if(!pkg||pkg.type!==CONTEXT_PACKAGE_TYPE||pkg.version!==CONTEXT_PACKAGE_VERSION||pkg.resourceScope!==CONTEXT_PACKAGE_RESOURCE_SCOPE||typeof text!=='string'||!['plain','markdown'].includes(format)||!Number.isSafeInteger(generation)||generation<0)invalid();
