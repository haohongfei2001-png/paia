...
export const { SUMMARY_FIELDS, ROW_FIELDS, MAX_ROWS, MAX_COUNT, emptyStructure, emptyRow, sanitizeStructure, sanitizeCaptureHealth, sanitizeIngestion } = globalThis.ArchiveDiagnostics;
...
export function sanitizeDiagnostics(value = {}) {
  ...
  const captureHealth=sanitizeCaptureHealth(value.captureHealth),ingestion=sanitizeIngestion(value.ingestion);
  return {
    ...(captureHealth?{captureHealth,captureHealthAt:timestamp(value.captureHealthAt)}:{}),
    ...(ingestion?{ingestion,ingestionAt:timestamp(value.ingestionAt)}:{}),
