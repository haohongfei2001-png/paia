...
export const configDefault=()=>({id:'memory:config',kind:'config',version:1,revision:0,defaultPolicy:'denied',budget:'standard',retentionDays:30,onboarded:false,userTouched:false,externalAccess:false,localOnly:false,includeUnorganizedInputs:false});
...
const fields={config:['id','kind','version','revision','defaultPolicy','budget','retentionDays','onboarded','userTouched','externalAccess','localOnly','includeUnorganizedInputs'],profile:['id','kind','version','profileId','name','budget','instruction','revision'],topic:['id','kind','version','profileId','topicId','decision','layoutGeneration'],entry:['id','kind','version','entryId','excluded'],input:['id','kind','version','inputId','excluded'],section:['id','kind','version','topicId','sectionId','excluded'],activity:['id','kind','version','createdAt','profileId','queryDigest','topicIds','entryIds','inputIds','action']};
export function validateMemoryRow(r){if(!r||typeof r!=='object'||Array.isArray(r)||r.version!==1||!Object.hasOwn(fields,r.kind)||Object.keys(r).some(k=>!fields[r.kind].includes(k))||typeof r.id!=='string'||!r.id.startsWith(MEMORY_PREFIX))return false;
 if(r.kind==='config')return r.id==='memory:config'&&r.defaultPolicy==='denied'&&isBudget(r.budget)&&[0,7,30,90].includes(r.retentionDays)&&Number.isSafeInteger(r.revision)&&r.revision>=0&&typeof r.onboarded==='boolean'&&typeof r.userTouched==='boolean'&&(r.externalAccess===undefined||typeof r.externalAccess==='boolean')&&(r.localOnly===undefined||typeof r.localOnly==='boolean')&&(r.includeUnorganizedInputs===undefined||typeof r.includeUnorganizedInputs==='boolean');
 ...
 if(r.kind==='input')return idOK(r.inputId)&&r.id===key('input',r.inputId)&&r.excluded===true;
 if(r.kind==='section')return idOK(r.topicId)&&idOK(r.sectionId)&&r.id===key('section',r.topicId,r.sectionId)&&r.excluded===true;
 return r.id.startsWith('memory:activity:')&&Number.isFinite(Date.parse(r.createdAt))&&idOK(r.profileId)&&/^[a-f0-9]{64}$/.test(r.queryDigest)&&Array.isArray(r.topicIds)&&r.topicIds.length<=1000&&r.topicIds.every(idOK)&&Array.isArray(r.entryIds)&&r.entryIds.length<=200&&r.entryIds.every(idOK)&&(r.inputIds===undefined||Array.isArray(r.inputIds)&&r.inputIds.length<=200&&r.inputIds.every(idOK))&&['build','copy','markdown','permission','profile','settings'].includes(r.action);
}
export const memoryMetaAllowed=id=>typeof id==='string'&&(id==='memory:config'||['profile','topic','entry','input','section','activity'].some(k=>id.startsWith('memory:'+k+':')));
...
export function policy(rows,profileId,temporary={},topics=null){
 ...
 const excluded=new Set(rows.filter(r=>r.kind==='entry').map(r=>r.entryId)),excludedInputs=new Set(rows.filter(r=>r.kind==='input').map(r=>r.inputId)),sections=new Set(rows.filter(r=>r.kind==='section').map(r=>JSON.stringify([r.topicId,r.sectionId])));
 ...
 return {rules,excluded,excludedInputs,sections,decision:id=>{if(never.has(id))return 'never';const d=rules.get(id);if(d==='denied'||d==='never')return d;return d==='allowed'||sessionAllowed(id)?'allowed':'default';},temporary:sessionAllowed};
}
