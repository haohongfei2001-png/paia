...
import {fail,prefix,refreshEntryIndex} from './thought-model.js';
import {nextSequence} from './thought-journal.js';
import {inputProjection} from './thought-evidence.js';

export async function enqueueInvalidation(store,t,inputId,reason,contentRevision) {
 ...
 await t.put('invalidations',{id,eventSchema:2,inputId,reason,contentRevision,sequence,at:store.clock(),stateKey:0,state:'pending',cursor:null,dependencyAck:false,organizerAck:false});
}
...
export async function invalidationBatch(store,limit=100) {
 ...
 const work=await store.run(()=>store.repository.transaction(false,async t=>{
  ...
  const page=await t.rangePage('dependencies','byInputTarget',prefix([event.inputId]),event.cursor,limit),input=await inputProjection(store,t,event.inputId);
  return {event,page,input};
 }));
 if(!work)return {processed:0,pending:false};
 const digests={};if(work.input){digests.body=await hashText(work.input.body);digests.note=await hashText(work.input.note);}
 ...
 return store.foundationWrite(async t=>{
  ...
  for(const {value:old}of work.page.rows) {
   ...
   const missing=!current||(current.lastRemovalSequence||0)>(dep.eligibilityEpochAtUse||0),changed=!missing&&(!dep.selectedFields?.length||dep.selectedFields.some(f=>digests[f]!==dep.fieldDigests?.[f]));
   if(dep.targetKind==='entry') {
    const row=await t.get('thoughts',dep.targetId);if(row) {
