...
function rowValid(row){
 return plain(row)&&row.version===VERSION&&KINDS.has(row.kind)&&validId(row.ownerId)&&validId(row.token)&&
  Array.isArray(row.sourceRecordIds)&&row.sourceRecordIds.length<=2000&&row.sourceRecordIds.every(validId)&&
  Number.isFinite(row.updatedAt)&&Number.isFinite(row.expiresAt)&&plain(row.operation);
}

export class RecoveryDraftStore{
 constructor(local,{clock=()=>Date.now(),ttlMs=DEFAULT_TTL_MS,maxBytes=DEFAULT_MAX_BYTES,maxDrafts=DEFAULT_MAX_DRAFTS,maxTotalBytes=DEFAULT_MAX_TOTAL_BYTES}={}){
 ...
 async save({kind,ownerId,token,operation,sourceRecordIds=[]}){
  ...
  existing.sort((a,b)=>b[1].updatedAt-a[1].updatedAt);let total=rowBytes,kept=1;
  for(const [k,,size]of existing){if(kept<this.maxDrafts&&total+size<=this.maxTotalBytes){total+=size;kept++;}else remove.push(k);}
  if(remove.length)await this.local.remove([...new Set(remove)]);
  await this.local.set({[ownKey]:row});return clone(row);
 }
 ...
 async prune(){
  ...
  rows.sort((a,b)=>b[1].updatedAt-a[1].updatedAt);let total=0,kept=0;
  for(const [k,row]of rows){const size=bytes(row);if(kept<this.maxDrafts&&total+size<=this.maxTotalBytes){total+=size;kept++;}else remove.push(k);}
  if(remove.length)await this.local.remove([...new Set(remove)]);
  return {kept,bytes:total,removed:new Set(remove).size};
 }
