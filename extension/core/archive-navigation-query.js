...
export function navigationRequest(options={}){
 ...
 if(kind==='providers'){if(key!==null)invalid();scope=K.rootScope();}
  ...
  const ref=kind==='project'?projectRef(options.projectRef):null;if(ref&&ref.providerKey!==key)invalid();scope=K.windowScope(key,kind,ref);
 }else invalid();
 ...
 return {scope,mode,limit,cursor,selectedDocumentId:options.selectedDocumentId==null?null:K.identifier(options.selectedDocumentId)};
}
const base=(q,selectedPath)=>({items:[],nextCursor:null,coverage:{state:'building'},generation:null,effectiveOrdering:'paia',unavailableReason:q.mode==='source'?'SOURCE_ORDER_UNAVAILABLE':null,selectedPath,cursorInvalid:false,oldReaderAvailable:true,operations:{metadataScanned:0,inputBodyReads:0}});
export class ArchiveNavigationQuery{
 constructor(store,options={}){this.store=store;this.index=new ArchiveNavigationIndex(store,options);}
 page(options={}){const q=navigationRequest(options);return this.store.run(async()=>{
  ...
  const gc=await this.index.collect(built.hash),prefix=K.pagePrefix(built.hash,built.state.active);
  return this.index.transaction(false,async t=>{
   ...
   result.generation=s.active;result.coverage={state:'complete',archiveComplete:true,total:s.count,scannedDocuments:c.scannedDocuments};
   if(q.cursor&&(q.cursor.scopeHash!==built.hash||q.cursor.generation!==s.active||q.cursor.mode!==q.mode||!q.cursor.lastKey.startsWith(prefix))){result.cursorInvalid=true;return result;}
   ...
   result.nextCursor=batch.next?JSON.stringify({version:1,scopeHash:built.hash,generation:s.active,lastKey:batch.next,mode:q.mode}):null;
   return result;
  },['meta','documents']);
 });}
 ...
 status(options={}){const q=navigationRequest(options);return this.store.run(async()=>{
  ...
  return this.index.transaction(false,async t=>{
   ...
   result.generation=s?.active||null;return result;
  });
 });}
}
