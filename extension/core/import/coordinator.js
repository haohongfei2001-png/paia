...
export class ImportCoordinator {
 constructor({transport,adapter=null,onProgress=()=>{}}={}){this.transport=transport;this.adapter=adapter;this.onProgress=onProgress;this.session=null;this.summary={phase:'idle'};this.busy=false;}
 ...
 async select(file,{consent=false,taskId}={}){await this.pause();allowed(file,{consent});this.session={file,consent,taskId,abort:new AbortController(),grant:null,fingerprint:null};return this.publish({phase:'selected'});}
 ...
 async projectedPass(s,method){
  let sequence=0,rows=[],bytes=0,projection=null,issues=0;
  const send=async()=>{
  ...
  const integrity=await inspectFile(s.file,{consent:s.consent,signal:s.abort.signal,...(this.adapter.profile?{candidateMode:'structural'}:{}),
   selectString:path=>projection?.selectString(path)===true,onEntry:()=>{projection=this.adapter.createProjection();},
 ...
 async preflight(){
  ...
  try{
   ...
   if(!this.adapter){const r=await inspectFile(s.file,{consent:true,signal:s.abort.signal});return this.publish({...r,phase:'unsupported',reason:'SCHEMA_UNVERIFIED'},s);}
   if(this.adapter.profile){
   ...
   const begun=await this.transport('begin',{fingerprint:s.fingerprint,adapterId:this.adapter.id,consent:true,...(s.taskId?{taskId:s.taskId}:{})});
 ...
 async commit(){
  const s=this.current();if(!this.adapter)fail('SCHEMA_UNVERIFIED');if(this.busy||!s.preflightComplete)fail('IMPORT_STATE');this.busy=true;
  try{
