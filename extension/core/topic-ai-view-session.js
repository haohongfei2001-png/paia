...
const clonePosition=value=>value?{scroll:value.scroll,cursor:value.cursor??null,pages:[...(value.pages||[])],query:value.query||''}:null;
...
export class TopicAIViewSession{
 ...
 remember(topicId,view,{scroll=0,cursor=null,pages=[],query=''}={}){if(!validTopic(topicId)||!VIEWS.has(view)||!Number.isFinite(scroll)||scroll<0||!Array.isArray(pages)||typeof query!=='string')throw new TypeError('INVALID_TOPIC_POSITION');const row=this.row(topicId,true),position={scroll,cursor,pages:[...pages],query};row.positions.set(view,position);return clonePosition(position);}
