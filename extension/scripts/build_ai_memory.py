"""Build v0.10.0 from current full regression and synthetic Chrome evidence."""
...
NAME = 'PAIA-v0.10.0-ai-memory-mvp'
LABEL = 'v0.10.0 AI Memory MVP'
def digest(path):
...
def package(output):
    ...
    if output!=ROOT/'outputs/v0100-artifacts':
    ...
    for kind in ['internal','release']:
        ...
        assert all((target/name).is_file() for name in ['core/memory/model.js','core/memory/service.js','core/memory/retrieval.js','ui/memory.js','AI_MEMORY.md'])
    ...
    receipt={'sourceCommit':revision,'version':'0.10.0','realExportVerified':False,'artifacts':artifacts}
...
if __name__=='__main__':
    if len(sys.argv)!=2:raise SystemExit('Usage: build_ai_memory.py OUTPUT_DIRECTORY')
    package(sys.argv[1])
