"""Build v0.9.2 from current full regression and synthetic Chrome evidence."""
...
NAME = 'PAIA-v0.9.2-product-hardening'
LABEL = 'v0.9.2 Product Hardening & Quality'
def digest(path):
...
def package(output):
    ...
    if output!=ROOT/'outputs/v092-artifacts':
        raise ValueError('Use the dedicated v092 artifact directory')
    ...
    receipt={'sourceCommit':revision,'version':'0.9.2','realExportVerified':False,'artifacts':artifacts}
...
if __name__=='__main__':
    if len(sys.argv)!=2:raise SystemExit('Usage: build_product_hardening.py OUTPUT_DIRECTORY')
    package(sys.argv[1])
