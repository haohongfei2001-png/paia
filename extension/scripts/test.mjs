...
let files=(await readdir('tests')).filter(n=>n.endsWith('.test.mjs')&&(!requested||group(n)===requested)).sort().map(n=>'tests/'+n);
...
if(audit.status===0&&developmentAudit.status===0&&!requested&&!shardSpec){const p='work/test-summary.json',r=JSON.parse(await readFile(p,'utf8'));r.fullSuite=true;r.auditPassed=true;r.testConcurrency=Number(concurrency);r.inputDigest=await inputDigest();if(r.inputDigest!==startingDigest)throw Error('SOURCE_CHANGED_DURING_TESTS');r.realGolden=await goldenBundle()?'AVAILABLE':'UNAVAILABLE';await writeFile(p,JSON.stringify(r,null,2)+'\n');}
