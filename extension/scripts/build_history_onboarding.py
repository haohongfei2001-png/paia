"""Build v0.9.0 from current full regression and synthetic Chrome evidence."""
...
NAME = 'PAIA-v0.9.0-history-onboarding'
LABEL = 'v0.9.0 History Completion & First-run Onboarding'
def digest(path):
...
def package(output):
    ...
    if output!=ROOT/'outputs/v090-artifacts':
        raise ValueError('Use the dedicated v090 artifact directory')
    ...
    receipt={'sourceCommit':revision,'version':'0.9.0','realExportVerified':False,'artifacts':artifacts}
...
if __name__=='__main__':
    if len(sys.argv)!=2:raise SystemExit('Usage: build_history_onboarding.py OUTPUT_DIRECTORY')
    package(sys.argv[1])
