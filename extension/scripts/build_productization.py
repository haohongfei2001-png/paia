"""Package the validated v0.8.0 productization checkpoint; never runs or deploys it."""
...
NAME = 'PAIA-v0.8.0-thought-library-productization'
LABEL = 'v0.8.0 Thought Library Productization'

def digest(path):
    return hashlib.sha256(path.read_bytes()).hexdigest()

def package(output):
    node = __import__('os').environ.get('PAIA_NODE', 'node')
    subprocess.run([node, 'scripts/compatibility-gate.mjs', '--sampler'], cwd=ROOT, check=True)
    acceptance = json.loads((ROOT / 'work/v080-product-e2e/acceptance.json').read_text())
    partial = json.loads((ROOT / 'work/v080-product-e2e/partial.json').read_text())
    migration = json.loads((ROOT / 'work/v080-product-e2e/migration.json').read_text())
    assert migration['sameExtensionId'] and migration['idempotent'] and migration['sourceInputEntryTopicRevisionEquality']
    assert acceptance['syntheticOnly'] and acceptance['visibleChrome'] and acceptance['inputCount'] >= 40
    assert set(acceptance['checks'] + [partial['check']]) == set('ABCDEFGHIJKLMNO')
    assert acceptance['externalRequests'] == 0 and not acceptance['errors']
    ...
    report = {'checkpoint':revision, 'version':manifest['version'], 'artifacts':artifacts,
              'releaseScope':'Release structure validated with isolated synthetic Chrome. Live DeepSeek response requires user-provided session credential.'}
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        raise SystemExit('Usage: python3 scripts/build_productization.py OUTPUT_DIRECTORY')
    package(sys.argv[1])
