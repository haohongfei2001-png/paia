import {normalizeSearch} from '../../core/search-ranking.js';
...
export function rankCandidate(item,query){
 const terms=queryTerms(query),q=normalizeSearch(query);
 const fields=[item.body,item.title||'',item.sectionTitle||'',item.topicName||''].map(normalizeSearch);
 const exact=q.length>=2&&(fields[0].includes(q)||fields[1]===q);
 let score=0;
 for(let i=0;i<terms.length;i++){
  const t=terms[i],w=weights?.get(t)||1;
  score+=w*(fields[0].includes(t)?3:0)+w*(fields[1].includes(t)?1:0)+w*(fields[2].includes(t)?.5:0)+w*(fields[3].includes(t)?.25:0);
 }
 return {...item,score:Math.max((exact?10000:0)+score,item.relatedScore||0),why:['与当前问题相关']};
}
let weights=null; // experiment-only invocation scoped; never packaged.
export function rankAll(items,query){
 const terms=queryTerms(query);weights=new Map();
 for(const t of terms){const df=items.filter(e=>normalizeSearch([e.body,e.title,e.sectionTitle,e.topicName].join(' ')).includes(t)).length;weights.set(t,Math.log(1+(items.length-df+.5)/(df+.5)));}
 return items.map(e=>rankCandidate(e,query)).filter(e=>e.score>0).sort(compareCandidates);
}
...
export function assemble(candidates,{query='',budget='standard',instruction='',removed=[]}={}){
 ...
 rankAll(candidates,query);const ranked=candidates.map(c=>({...rankCandidate(c,query),state:semanticState(c)})).filter(c=>!excluded.has(c.id)&&(!normalizeSearch(query)||terms.length&&c.score>0)&&(history||c.type!=='possibleEvolution')).sort(compareCandidates);
 const unique=[],bodies=new Map();
 for(const c of ranked){const sig=JSON.stringify([normalizeSearch(c.body),c.topicId,c.sectionId||c.sectionTitle||'',c.title||'',c.type,c.time,c.state,c.excerpted?c.id:'']);if(!c.body.trim())continue;const previous=bodies.get(sig);if(previous){const refs=[...new Set([...previous.evidenceEntryIds,...c.evidenceEntryIds])],priority=x=>x.human?0:x.kind==='entry'?1:2,score=Math.max(previous.score,c.score);if(priority(c)<priority(previous))Object.assign(previous,c);previous.score=score;previous.evidenceEntryIds=refs;continue;}bodies.set(sig,c);unique.push(c);}
 ...
 const warnings=contextWarnings(items),text=contextText(items,{instruction,warnings});return {items,text,warnings,characters:[...text].length,tokens:estimatedTokens(text),budget,limits:limit,matched:unique.length,omitted:Math.max(0,unique.length-items.length),localOnly:true};
}
