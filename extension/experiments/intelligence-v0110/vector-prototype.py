...
t=time.perf_counter();model=StaticModel.from_pretrained('/private/tmp/paia-v0110-model');load_ms=(time.perf_counter()-t)*1000
...
queries=data['queries'];t=time.perf_counter();qv=model.encode([q['query'] for q in queries],use_multiprocessing=False);encode_ms=(time.perf_counter()-t)*1000
