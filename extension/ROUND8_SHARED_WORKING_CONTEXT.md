...
The first full Round 8 GitHub Actions run passed all 878 portable tests and both package audits, but several browser journeys stopped before exercising product logic because the test selector `[data-view=settings]` became ambiguous after Round 6 added a second Settings entry in the Thought Library empty state. The browser harness now scopes Settings navigation to the primary sidebar (`.sidebar [data-view=settings]`). This is test-only hardening; runtime product behavior is unchanged.

A subsequent browser run exposed three additional deterministic-harness issues rather than privacy-policy relaxations: the backup fixture's absolute-path filter accidentally matched GitHub Runner's own `/work/` prefix; an older Memory journey clicked a detail node before its authorization refresh settled; and the new direct-Input journey treated a normal fail-closed `MEMORY_STALE` during capture settling as a terminal assertion. Those fixtures now scope the backup filter relative to the extension root, wait for the authorization state before opening details, and retry only the initial local build while its captured generation settles. Product stale/share checks remain fail-closed and unchanged. A fresh full Round 8 verification is required on this candidate before development closure.
