...
const reportEl=document.getElementById('gate-report');
...
async function inspectPAIALibraryReadOnly(){
 const base={diagnosticVersion:2,readOnly:true,complete:false};
 ...
 try{
  ...
  const report=await new Promise((resolve,reject)=>{
   ...
   const scan=(r,kind,visit)=>{r.onsuccess=()=>{const cursor=r.result;if(!cursor)return;if(report.scanned[kind]>=report.limits.maxRowsPerScan||Date.now()>=deadline){limited=true;return;}report.scanned[kind]++;try{visit(cursor);cursor.continue();}catch{failed=true;tx.abort();}};};
   ...
   scan(topics.openCursor(),'topics',cursor=>{
    ...
    if(!generation(row.activeLayoutGeneration)){report.layouts[row.activeLayoutGeneration==null?'missingGeneration':'invalidGeneration']++;candidates.set(row.id,{sequence:row.layoutSequence,job:!!row.layoutJobId});}
   });
   if(topics.indexNames.contains('byIndex')){
    ...
    if(report.index.expectedKeyPath){const range=IDBKeyRange.bound([0],[0,[]],false,true);request(index.count(range),value=>report.index.activeEntries=value);scan(index.openKeyCursor(range),'indexKeys',cursor=>indexedIds.add(cursor.primaryKey));}
   }
   if(present.sections){const store=tx.objectStore('sections');request(store.count(),value=>report.totals.sections=value);scan(store.openCursor(),'sections',cursor=>{const row=cursor.value;if(row.lifecycle!=='active'||row.redirectTo||!generation(row.layoutGeneration))return;let set=sections.get(row.topicId);if(!set)sections.set(row.topicId,set=new Set());set.add(row.layoutGeneration);});}
   if(present.placements)request(tx.objectStore('placements').count(),value=>report.totals.placements=value);
   if(present.meta){const store=tx.objectStore('meta');request(store.get('library-documents-compat-v2'),row=>{report.markers.compatibilityComplete=row?.complete===true;report.markers.compatibilityUnresolvedAtMigration=Number.isSafeInteger(row?.unresolvedLayouts)&&row.unresolvedLayouts>=0?row.unresolvedLayouts:0;});request(store.getKey('aiProductizationMigration'),key=>report.markers.aiProductizationPresent=key!==undefined);}
   tx.oncomplete=()=>{clearTimeout(timer);if(failed)return;report.index.activeTopicsSeen=[...activeIds].filter(id=>indexedIds.has(id)).length;report.complete=!limited;report.status=limited?'partial':'complete';if(!limited&&report.index.expectedKeyPath)report.index.gap=report.topics.active-report.index.activeTopicsSeen;for(const [id,row]of candidates){const evidence=sections.get(id);if(row.job)report.layouts.jobBlocked++;else if(generation(row.sequence)&&evidence?.has(row.sequence))report.layouts.sequenceWithSection++;else if((row.sequence==null||row.sequence===1)&&evidence?.has(1))report.layouts.legacyOneWithSection++;else report.layouts.unresolvedCandidates++;}activeIds.clear();indexedIds.clear();candidates.clear();sections.clear();resolve(report);};
   tx.onabort=()=>{clearTimeout(timer);reject(new Error('READ_UNAVAILABLE'));};tx.onerror=()=>{};
  });
...
async function run(){
 statusEl.textContent='Running the local read-only check…';detailEl.textContent='Keep this page open for a few seconds.';copyButton.disabled=true;payload='';reportEl.textContent='';
 const report=await inspectPAIALibraryReadOnly();
 payload='PAIA_READONLY_DIAGNOSTIC '+JSON.stringify(report);
 reportEl.textContent=JSON.stringify(report,null,2);
 copyButton.disabled=false;
 if(report.status==='complete'&&report.complete===true){statusEl.textContent='Read-only check complete';detailEl.textContent='No repair was performed. Copy the result and send it back to ChatGPT for interpretation.';}
 else{statusEl.textContent='Read-only check did not fully complete';detailEl.textContent='Do not repair, reinstall, clear data, or load another PAIA copy. Copy this result so the failure can be diagnosed safely.';}
 console.info(payload);
}
copyButton.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(payload);copyButton.textContent='Copied';}catch{copyButton.textContent='Copy failed — select the line from DevTools only if needed';}});
retryButton.addEventListener('click',()=>void run());
void run();
