...
The session collects plain text without changing the Source. It debounces ordinary saves, protects an unacknowledged draft before sending an edit, and sends `EDIT_LIBRARY_BATCH` with an operation ID plus expected entry, field and Input revisions. A lost acknowledgement may reuse that operation ID. An external revision may update untouched fields; a changed field with a local draft becomes a conflict and is not overwritten. The comparison shows the draft and latest saved values. Saving the draft against the latest revision requires an explicit user choice, adopts remote values for fields the user did not edit, and keeps the revision precondition. A navigation or panel close must flush or visibly retain the draft. IME composition cannot be treated as a completed save.
